...
# App registration client secret authentication parameters
$ClientId = "<APPCLIENTID>" # App registration client ID
$ClientSecret = "<APPCLIENTSECRET>" # App registration client secret
...
# Fix for macOS/Linux compatibility - use appropriate temp directory and path separators
$LogPath = if ($IsRunningInAutomation) { 
    $null 
} else { 
    # Detect operating system and use appropriate paths
    if ($PSVersionTable.Platform -eq "Unix" -or $env:HOME) {
        # macOS/Linux
        $tempDir = if ($env:TMPDIR) { $env:TMPDIR } elseif ($env:TMP) { $env:TMP } else { "/tmp" }
        # Ensure directory exists and is writable
        if (-not (Test-Path -Path $tempDir)) {
            New-Item -Path $tempDir -ItemType Directory -Force | Out-Null
        }
        Join-Path -Path $tempDir -ChildPath "CalendarEventCreation_$((Get-Date -Format 'yyyyMMdd_HHmmss')).log"
    } else {
        # Windows
        "$env:TEMP\CalendarEventCreation_$((Get-Date -Format 'yyyyMMdd_HHmmss')).log"
    }
}
$MaxRetryCount = 3
...
###########################################################################
# Function for logging - Azure Automation compatible with macOS support
###########################################################################
function Write-Log {
    param (
    ...
    if ($IsRunningInAutomation) {
        ...
        # Append to log file if not in Azure Automation and LogPath is valid
        if ($LogPath) {
            try {
                # Test if we can write to the log path
                $testDir = Split-Path -Path $LogPath -Parent
                ...
                # This prevents the script from failing due to file system issues
                if ($Level -eq "ERROR") {
                    Write-Host "[WARNING] Could not write to log file: $($_.Exception.Message)" -ForegroundColor Yellow
                    Write-Host "[INFO] Continuing with console logging only..." -ForegroundColor White
                }
                # Set LogPath to null to prevent further attempts
                $script:LogPath = $null
            }
        }
    }
...
function Assert-GraphAuth {
    ...
    if ($MinutesSinceAuth.TotalMinutes -ge $MaxTokenMinutes) {
        ...
        Write-Log "Connecting to Microsoft Graph using client secret authentication"
        ...
        # Connect to Graph API with client secret authentication
        Connect-MgGraph -TenantId $TenantId -ClientSecretCredential $ClientCredential
        
        $Global:LastGraphAuthTime = Get-Date
        Write-Log "Successfully reconnected to Microsoft Graph using client secret authentication"
    }
}
...
function Connect-ExchangeOnlineWithAppAuth {
    try {
        Write-Log "Connecting to Exchange Online using client secret authentication"
        
        if (-not [string]::IsNullOrEmpty($ClientId) -and -not [string]::IsNullOrEmpty($ClientSecret) -and -not [string]::IsNullOrEmpty($TenantDomain)) {
            # Use client secret authentication with tenant domain (not GUID)
            $SecureClientSecret = ConvertTo-SecureString -String $ClientSecret -AsPlainText -Force
            $ClientCredential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList $ClientId, $SecureClientSecret
            
            Connect-ExchangeOnline -AppId $ClientId -ClientSecret $SecureClientSecret -Organization $TenantDomain -ShowProgress:$false -ErrorAction Stop
        }
        else {
            throw "Missing required parameters for Exchange Online authentication. ClientId, ClientSecret, and TenantDomain are required."
        }
        
        $Global:ExchangeConnected = $true
        $Global:LastExchangeAuthTime = Get-Date
        Write-Log "Successfully connected to Exchange Online using client secret authentication"
    }
    catch {
...
###########################################################################
# Authenticate with Microsoft Graph using client secret
###########################################################################
try {
    Write-Log "Connecting to Microsoft Graph API using client secret authentication"
    
    # Convert client secret to secure string and create credential
    $SecureClientSecret = ConvertTo-SecureString -String $ClientSecret -AsPlainText -Force
    $ClientCredential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList $ClientId, $SecureClientSecret
    ...
    $Global:LastGraphAuthTime = Get-Date
    Write-Log "Connected to Microsoft Graph API using client secret authentication"
    ...
    Write-Log "Failed to connect to Microsoft Graph API using client secret authentication: $_" -Level "ERROR"
    throw "Failed to connect to Microsoft Graph API using client secret authentication: $_"
}
...
function Connect-ToMicrosoftGraph {
    ...
    try {
        ...
        if ((-not $Context) -or $Force) {
            Write-Log "Connecting to Microsoft Graph with client secret authentication..." -Level "INFO"
            ...
            Connect-MgGraph -TenantId $TenantId -ClientSecretCredential $ClientCredential -ErrorAction Stop
